import { useState,useEffect } from "react"
import { db } from "../firebase/config"
import { doc,getDoc,updateDoc } from "firebase/firestore"
import {useParams,useNavigate} from 'react-router-dom'
import { exists } from "firebase/firestore/pipelines"




function Edit(){
    const {id} = useParams() // /edit/:id   
    const navigate = useNavigate()

    const [title,setTitle] = useState('')
    const [content,setContent] = useState('')
    const [loading,setLoading] = useState(true)
    const [updating,setUpdating] = useState(false)

    useEffect(()=>{
        async function getBlog() {
            try {
                const docRef = doc(db,'blogs',id) 
                const docSnap = await getDoc(docRef)

                if(docSnap.exists()){
                    setTitle(docSnap.data().title)
                    setContent(docSnap.data().content)
                }else{
                    console.log('no such documet found');
                    navigate('/')
                    
                }
            } catch (error) {
                console.error(error); 
            }finally{
                setLoading(false)

            } 
        }
        getBlog()
    },[id,navigate])

    async function handleUpdate(e) {
        e.preventDefault()
        if(!title.trim() || !content.trim())return

        setUpdating(true)
        try {
            const docRef = doc(db,'blogs',id)
            await updateDoc(docRef,{
                title:title,
                content:content
            })
            navigate('/')
        } catch (error) {
            console.error(error);
            alert('Error updating the Blog')
            
        }finally{
            setLoading(false)
        }
    }

    const style ={
        container: {
            maxWidth: '650px',
            margin: '50px auto',
            padding: '40px 30px',
            fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif',
            borderRadius: '12px',
            boxShadow: '0 8px 24px rgba(0, 0, 0, 0.06)',
            backgroundColor: '#ffffff',
            border: '1px solid #e2e8f0'
        },
        heading: {
            marginTop: 0,
            marginBottom: '24px',
            color: '#1a202c',
            fontSize: '24px',
            fontWeight: '600'
        },
        inputGroup: {
            marginBottom: '20px',
        },
        label: {
            display: 'block',
            marginBottom: '8px',
            fontSize: '14px',
            fontWeight: '500',
            color: '#4a5568'
        },
        input: {
            width: '100%',
            padding: '12px 16px',
            backgroundColor:' #f4f4f4',
            borderRadius: '8px',
            border: '1px solid #cbd5e1',
            fontSize: '16px',
            boxSizing: 'border-box',
            outline: 'none',
            color: '#2d3748',
            fontFamily: 'inherit'
        },
        textarea: {
            width: '100%',
            padding: '12px 16px',
            borderRadius: '8px',
            backgroundColor:' #f4f4f4',
            border: '1px solid #cbd5e1',
            fontSize: '15px',
            boxSizing: 'border-box',
            outline: 'none',
            color: '#2d3748',
            fontFamily: 'inherit',
            resize: 'vertical',
            lineHeight: '1.6'
        },
        btnGroup: {
            display: 'flex',
            gap: '12px',
            marginTop: '25px'
        },
        saveBtn: {
            padding: '12px 24px',
            cursor: 'pointer',
            fontSize: '15px',
            fontWeight: '600',
            backgroundColor: '#3182ce',
            color: '#fff',
            border: 'none',
            borderRadius: '8px',
            transition: 'background-color 0.2s',
            opacity: updating ? 0.7 : 1
        },
        cancelBtn: {
            padding: '12px 24px',
            cursor: 'pointer',
            fontSize: '15px',
            fontWeight: '500',
            backgroundColor: '#fff',
            color: '#4a5568',
            border: '1px solid #cbd5e1',
            borderRadius: '8px',
            transition: 'all 0.2s'
        },
        statusMessage: {
            padding: '40px',
            textAlign: 'center',
            fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif',
            color: '#718096'
        }
    }


    if(loading)return <p style={style.statusMessage}>Loading blog post data...</p>;

    return(
         <div style={style.container}>
            <h2 style={style.heading}>Edit Your Blog Post</h2>
            <form onSubmit={handleUpdate}>
                <div style={style.inputGroup}>
                    <label style={style.label}>Title</label>
                    <input 
                        type="text"
                        value={title}
                        onChange={(e)=>setTitle(e.target.value)} 
                        required
                        style={style.input}
                        /> 
                </div>
                <div style={style.inputGroup}>
                    <label style={style.label}>Content</label>
                    <textarea 
                        type="text"
                        value={content}
                        onChange={(e)=>setContent(e.target.value)}
                        required
                        rows='8'
                         style={style.textarea}
                        /> 
                </div>

            <div style={style.btnGroup}>  
                <button
                    type="submit"
                    disabled={updating}
                style={style.saveBtn} 
                >{updating?'Saving chnages ...':'Update'}</button>


                <button
                    type="button"
                    onClick={()=>navigate('/')}
                    style={style.cancelBtn}
                >Cancel</button>
            </div>
            </form>

         </div>
    )
}
export default Edit